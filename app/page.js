...
export default function Home() {
  ...
  const ourArticles = [
    {
      id: 'our-1', region: 'OurArticles', title: "Eluru lo New Super Specialty Hospital Opening Next Month",
      category: "Local", summary: "Eluru govt hospital ki 100 kotlu tho new building kattaru. 200 beds extra.",
    ...
    {
      id: 'our-2', region: 'OurArticles', title: "AP lo 50,000 Govt Jobs Notification Released",
      category: "Jobs", summary: "APPSC released notification for Group 1, Group 2 and Group 3 posts.",
  ...
  return (
    <main style={{width:'100%', background:colors.bg, color:colors.text, minHeight:'100vh', fontFamily:'system-ui'}}>
      ...
      <header style={{position:'sticky', top:0, zIndex:10, background:colors.bg, borderBottom:`1px solid ${colors.border}`}}>
        ...
        <div style={{maxWidth:'1200px', margin:'0 auto', padding:'16px 20px'}}>
          <input
            ...
            style={{width:'100%', padding:'12px 16px', borderRadius:'10px', border:`1px solid ${colors.border}`, background:colors.card, color:colors.text, fontSize:'15px'}}
          />
        </div>
          ...
          {tabs.map(tab => (
            <button
              ...
              style={{
                ...
                borderRadius:'8px',
                border:'none',
                background: activeTab === tab? '#3b82f6' : colors.card,
      ...
      <section style={{maxWidth:'1200px', margin:'0 auto', padding:'32px 16px'}}>
        ...
        {loading?
          <p style={{textAlign:'center', fontSize:'18px'}}>Loading Live News...</p> :
          <div style={{display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(320px, 1fr))', gap:'24px'}}>
            {filteredArticles.map(article => (
              <div key={article.id} style={{background:colors.card, padding:'20px', borderRadius:'16px', border:`1px solid ${colors.border}`, display:'flex', flexDirection:'column'}}>
                ...
                <a href={article.url} target="_blank" style={{color:'#3b82f6', fontWeight:'600', marginBottom:'12px', textDecoration:'none'}}>Read More →</a>
                ...
                <div style={{borderTop:`1px solid ${colors.border}`, paddingTop:'12px'}}>
                  <input
                    ...
                    style={{width:'100%', padding:'8px', borderRadius:'6px', border:`1px solid ${colors.border}`, background:colors.bg, color:colors.text, marginBottom:'8px'}}
                  />
